@component('mail::message')
    ...
    Wil je je inschrijving wijzigen, <a href="{{ route('signin.edit', [$mensaUser->confirmation_code]) }}">klik dan
        hier</a>.
    ...
    @component('mail::table')
        ...
        @endforeach
        | &nbsp;<br/>Met voor jou een totaal bedrag van: | &nbsp;
        <br/>&euro;{{ number_format(!$mensaUser->isStaff()?$mensaUser->price():0, 2) }} |
    @endcomponent
    @if($mensaUser->isStaff())
        <i>Je bent ingeschreven
            als {{ $mensaUser->cooks?"koker":"" }} {{ $mensaUser->dishwasher?($mensaUser->cooks?"en als ":"")."afwasser":"" }}
            en hoeft daarom niet te betalen!</i>
        <br/>
    @endif
    ...
    @if($mensaUser->intros()->count())
        ...
        @component('mail::table')
            ...
            @endforeach
            | &nbsp;<br/>Met voor je intro een totaal bedrag van: | &nbsp;
            <br/>&euro;{{ number_format(!$mensaIntro->isStaff()?$mensaIntro->price():0, 2) }} |
        @endcomponent
        @if($mensaIntro->isStaff())
            <i>Je bent ingeschreven
                als {{ $mensaIntro->cooks?"koker":"" }} {{ $mensaIntro->dishwasher?($mensaIntro->cooks?"en als ":"")."afwasser":"" }}
                en hoeft daarom niet te betalen!</i>
            <br/>
        @endif
