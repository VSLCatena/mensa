...
function getDay(date: Date, language: Language, preferShort = false): string {
  const days = preferShort ? Lang.date.days.short : Lang.date.days.long;
  switch (date.getDay()) {
    case 0:
...
function getMonth(date: Date, language: Language, preferShort = false): string {
  const months = preferShort ? Lang.date.months.short : Lang.date.months.long;
