...
class GenerateTokenController extends Controller
{
    ...
    public function __invoke(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $log->text = "GenerateToken/userLookup/getUpdatedUser: ClientException with ". $user->name . " (" . $azureUser->principal_name . ")";
            $log->save();
        ...
        if ($user == null) {
            ...
            $log->user_id = $this->systemUser->id;
            $log->object_id = $azureUser->id;
            $log->text = "GenerateToken/userLookup/getUpdatedUser: user not found for" . $azureUser->principal_name;
            $log->save();
        ...
        $log->user_id = $this->systemUser->id;
        $log->object_id = $azureUser->id;
        $log->text = "GenerateToken/userLookup/getUpdatedUser: Token created for " . $user->name . " (" . $azureUser->principal_name . ")";
        $log->save();
